...
module BlurbHelper
  ...
  def blurb(headline, clean_body, blurb_length = Settings.channel.blurb_length)
    ...
    blurb = clean_body[start_blurb_at .. clean_body.length]
              .truncate(blurb_length, separator: ' ', omission: Settings.channel.blurb_omission)
              .gsub(/\W#{ Settings.channel.blurb_omission }$/, '')
    [headline, blurb]
  end

  def citation_blurb(clean_body, blurb_length = Settings.channel.citation_blurb_length)
    return [clean_body, nil] if clean_body.scan(/\-\-/).empty?
    citation_text = Array(clean_body.scan(/^(.*?)\-\-/).first).first
                                    .truncate(blurb_length, separator: ' ', omission: Settings.channel.blurb_omission)
                                    # .gsub(/\W*#{ Settings.channel.blurb_omission }\Z/m, '') REVIEW

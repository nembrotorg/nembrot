...
describe 'Citations' do
  ...
  describe 'index page' do
    ...
    it 'has a link to the citation' do
      page.should have_selector('a', citation_path(@current_channel, @citation.id))
    end
  end

  describe 'show page' do
    before do
      visit citation_path(@current_channel, @citation)
    end
    it 'has the citation title' do
      page.should have_selector('h1', @citation.headline)
    end
    it 'has a link to the book cited' do
      page.should have_selector('a', book_path(@current_channel, @book.slug))
    end
